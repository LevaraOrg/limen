...
machine*. Whether Circlead may have 8080 depends on whether Tessera already has
...
```caddyfile
# Generated by limen 0.13.0 — do not edit; regenerate with `limen ports --caddy`.
...
`--write` is meant to be run often and reloaded rarely. It replaces the file
only when the content actually differs and says which of the two happened, and
it writes through a rename, so a reader never sees half a configuration. When
the table has a conflict it writes **nothing** and exits 1 — with a watching
proxy the difference between a warning and an outage is exactly that refusal.
...
`caddy run --watch` re-adapts its configuration when the file changes — and it
follows `import`, so a regenerated `limen.caddy` is enough and no `reload` is
needed. Under Homebrew that flag goes into the LaunchAgent:
...
which is why its first line says so. `.localhost` names need no `/etc/hosts`
